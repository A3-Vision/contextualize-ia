...
3. Arquivos **`app/`**, **`*.html`**, **`landing.css`**, **`diagnostico*.json`** são a interface do pacote no navegador — **não** precisam ser recriados no repositório de contexto do usuário.
